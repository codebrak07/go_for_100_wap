...
1: ["JavaScript data types define the type of value a variable can store and how that value behaves during program execution JavaScript divides data types into two main categories called Primitive and Non Primitive types Primitive data types represent single simple values and are immutable meaning their actual stored value cannot be modified directly when you change them JavaScript actually creates a new value instead Primitive types include string number boolean undefined null bigint and symbol string represents textual data number represents both integers and floating point numbers boolean stores true or false values used for decision making undefined means a variable has been declared but has not yet been assigned a value null represents an intentional absence of value bigint is used for extremely large integers beyond the normal number limit and symbol represents unique identifiers often used in advanced object properties Non primitive types include objects arrays and functions which can store multiple related values and complex structures typeof operator is commonly used to inspect the type of a variable at runtime since JavaScript is dynamically typed meaning variables do not require explicit type declaration and their type can change during execution understanding data types is fundamental because operations such as addition comparison or function behavior depend heavily on the underlying data type"],

2: ["Variables in JavaScript act as named containers that store values used by the program during execution JavaScript provides three main ways to declare variables which are var let and const each having different scope rules and behavior var was the original way to declare variables and it is function scoped meaning it is accessible anywhere within the function in which it is declared however var is also hoisted meaning its declaration is moved to the top of the scope during compilation which can lead to confusing bugs and unintended behavior because variables may appear usable before their actual declaration let was introduced in modern JavaScript to solve many issues with var it is block scoped meaning it only exists inside the block defined by curly braces such as loops or conditionals let allows reassignment of values but does not allow redeclaration in the same scope const is also block scoped but must be assigned a value at the moment it is declared and cannot be reassigned afterwards however if the value stored in const is an object or array its internal properties or elements can still be modified because const only prevents reassignment of the variable reference not mutation of the stored object best practice in modern JavaScript development is to use const by default and only use let when you know the variable value will need to change this practice improves code safety and prevents accidental reassignment errors"],

3: ["Conditional statements allow a program to make decisions by executing different blocks of code depending on whether certain conditions evaluate to true or false the most basic conditional structure is the if statement which executes a block of code only when a specified condition evaluates to true conditions are usually expressions that produce boolean values such as comparisons using operators like greater than less than equal to or logical operators like AND OR and NOT when the if condition evaluates to false the optional else block can execute an alternative piece of code for situations where multiple different conditions must be evaluated developers can use else if blocks which allow testing additional conditions sequentially until one evaluates to true JavaScript also provides a ternary operator which is a shorter expression based alternative to if else syntax written as condition ? valueIfTrue : valueIfFalse and is often used when assigning values based on simple conditions for scenarios where a variable must be compared against many fixed possible values developers often use the switch statement which evaluates the value once and compares it against multiple case labels the break keyword is used to stop execution after a matching case otherwise execution continues into the next case which is known as fallthrough a default block may also be included to execute when none of the cases match conditional logic is essential in programming because it allows applications to respond differently depending on user input data conditions or program state"],

4: ["Loops are programming constructs that allow the same block of code to be executed repeatedly until a certain condition is met they are extremely important when working with collections of data such as arrays lists or repeated computations one of the most common loop structures is the for loop which consists of three parts initialization condition and update initialization usually sets the starting value of a counter variable the condition determines how long the loop should continue running and the update expression changes the counter after each iteration while loops are another looping structure that repeatedly executes code as long as the specified condition remains true unlike the for loop a while loop typically requires the counter variable to be declared outside the loop and manually updated inside it JavaScript also includes the do while loop which is similar to the while loop but guarantees that the loop body executes at least once because the condition is checked only after the first execution loops often include control keywords such as break which immediately terminates the loop and continue which skips the remainder of the current iteration and jumps to the next iteration loops must be written carefully because if the loop condition never becomes false it can result in an infinite loop which may freeze the application or crash the program loops are commonly used to iterate through arrays process data sets perform repeated calculations or generate dynamic content"],

5: ["Functions are one of the most fundamental building blocks in JavaScript because they allow developers to group reusable logic into a single callable unit a function is essentially a block of code designed to perform a specific task and it executes only when it is invoked or called functions help break large programs into smaller manageable pieces making code easier to read maintain and debug JavaScript supports several ways to define functions the traditional method is the function declaration syntax written as function functionName() { } which creates a named function that can often be called even before its definition due to hoisting another approach is the function expression where a function is assigned to a variable such as const add = function(a b) { return a + b } modern JavaScript frequently uses arrow functions which provide a shorter syntax like const add = (a b) => a + b arrow functions are especially useful for short operations and callbacks functions may accept inputs known as parameters and return outputs using the return keyword when a return statement is executed the function immediately stops running and sends the specified value back to the caller if no return statement exists the function automatically returns undefined functions enable abstraction meaning developers can hide complex logic behind simple function calls making programs easier to understand reuse and extend across different parts of an application"],

6: ["Parameters and return values allow functions to accept input and produce output parameters are variables defined inside the function declaration that act as placeholders for incoming values when a function is called the actual values passed are called arguments functions can accept multiple parameters separated by commas and JavaScript does not enforce strict type checking meaning any type of value can be passed into parameters functions may return a value using the return keyword which sends the result back to the code that called the function once the return statement executes the function stops running immediately if a function does not explicitly return a value JavaScript automatically returns undefined modern JavaScript also supports default parameters which allow assigning default values when arguments are not provided and rest parameters which allow functions to accept unlimited arguments using syntax like function(...args) which collects them into an array understanding parameters and return values is important because they enable modular reusable code and allow functions to communicate results back to other parts of the program"],

7: ["Arrays are ordered collections used to store multiple values in a single variable instead of creating separate variables for each value arrays are especially useful when working with lists of data such as numbers names or objects elements in an array are stored in sequential order and accessed using an index starting from zero meaning the first element is at position 0 arrays in JavaScript are dynamic meaning their size can grow or shrink during runtime common array methods include push which adds an element to the end of the array pop which removes the last element shift which removes the first element and unshift which adds an element at the beginning arrays also have a length property which indicates the total number of elements arrays can store values of any type including numbers strings objects or even other arrays arrays are widely used in programming because they allow efficient storage processing and iteration of grouped data using loops or higher order functions"],

8: ["Objects are one of the most important data structures in JavaScript used to store structured data using key value pairs each key acts as a property name and each value represents the associated data values can be any valid JavaScript type including numbers strings arrays functions or even other objects objects are defined using curly braces and properties are separated by commas properties can be accessed using dot notation such as object.property or bracket notation such as object['property'] bracket notation is especially useful when property names are dynamic objects are commonly used to represent real world entities such as users products or configurations because they allow grouping related information into a single structure properties can be added modified or removed dynamically using assignment or the delete keyword making objects extremely flexible objects are fundamental in JavaScript because almost everything including arrays functions and classes is built upon the object system"],

9: ["An array of objects is a powerful data structure that combines the features of arrays and objects and is commonly used when working with collections of structured records for example when storing a list of users products or database entries each element in the array represents one object containing multiple related properties such as name age email or price this structure allows applications to manage large datasets efficiently developers first access the desired object using the array index and then access the object's properties using dot notation arrays of objects are widely used when processing API responses rendering lists in user interfaces performing filtering sorting or searching operations and storing structured data in memory understanding this structure is critical for real world development because most backend services and databases return data in this format"],

10: ["Destructuring is a modern JavaScript feature that allows developers to extract values from arrays or properties from objects and assign them directly to variables using concise syntax instead of manually accessing each value destructuring reduces repetitive code and improves readability for arrays destructuring works by matching variable positions with array indices for example let [a b] = arr assigns the first element to a and the second to b for objects destructuring works by matching variable names with property names for example let {name age} = user extracts those properties from the object destructuring also supports default values variable renaming and nested extraction making it extremely powerful when working with complex data structures destructuring is widely used in modern frameworks APIs and functional programming patterns because it simplifies working with structured data"],

11: ["Spread and rest operators both use the same three dots syntax but perform different tasks spread operator expands elements of an array or properties of an object into individual elements for example using [...arr] creates a shallow copy of an array while {...obj} creates a shallow copy of an object spread operator is commonly used when merging arrays copying objects or passing multiple arguments into functions rest operator on the other hand collects multiple values into a single array and is typically used in function parameters such as function(...args) where args becomes an array containing all passed arguments rest parameters are especially useful when the number of arguments is unknown both spread and rest operators greatly simplify working with collections of data and reduce the need for manual looping or argument handling"],

12: ["Higher order functions are functions that either take other functions as arguments or return functions as their result JavaScript treats functions as first class citizens which means functions can be assigned to variables passed into other functions and returned from functions this ability enables powerful programming patterns such as callbacks functional composition and abstraction callback functions are functions passed into another function so they can be executed later for example when handling events timers or asynchronous operations higher order functions make code more reusable modular and expressive many built in JavaScript features such as array methods event listeners and promise handlers rely heavily on higher order functions understanding this concept is important for writing modern JavaScript especially when working with asynchronous programming or functional programming styles"],

13: ["Array higher order functions provide powerful and expressive ways to process arrays without writing manual loops these methods accept callback functions which define how each element should be processed common methods include map which transforms each element and returns a new array filter which returns a new array containing only elements that satisfy a condition reduce which processes elements and combines them into a single accumulated value find which returns the first element that satisfies a condition includes which checks if a value exists in the array and sort which arranges elements according to a comparison function these methods make code shorter more readable and easier to maintain compared to traditional looping structures they also encourage a functional programming approach where data transformations are expressed clearly through chained operations"],

14: ["JavaScript execution model is based on a single threaded architecture meaning it can only execute one operation at a time the call stack is responsible for tracking function execution order when a function is called it is pushed onto the stack and when it finishes it is removed asynchronous operations such as network requests timers or file operations are handled outside the call stack by browser APIs or runtime environments the event loop continuously checks whether the call stack is empty and if there are pending tasks waiting in queues JavaScript uses two main queues the task queue which stores callbacks from timers or events and the microtask queue which stores promise callbacks the microtask queue has higher priority than the task queue this system allows JavaScript to perform asynchronous operations without blocking the main execution thread ensuring responsive applications"],

15: ["setTimeout and setInterval are timer functions provided by the browser or JavaScript runtime environment setTimeout schedules a function to run once after a specified delay measured in milliseconds setInterval repeatedly executes a function at regular intervals until it is stopped using clearInterval both functions are often used for delayed actions animations polling operations or periodically updating user interfaces clearTimeout can cancel a scheduled timeout before it executes developers should remember that timer delays are not guaranteed to run at the exact time specified because they are placed in the task queue and will only execute when the call stack is free understanding timers is important for handling asynchronous behavior in user interfaces and interactive applications"],

16: ["Promises are objects used to represent the eventual completion or failure of asynchronous operations a promise begins in a pending state and later transitions into either fulfilled when the operation succeeds or rejected when an error occurs promises provide methods such as then to handle successful results catch to handle errors and finally to run code regardless of success or failure promises help solve the problem of deeply nested callbacks known as callback hell by allowing asynchronous operations to be chained together in a readable way promises are widely used in modern JavaScript especially when working with APIs network requests or asynchronous computations and they form the foundation for newer syntax such as async and await"],

17: ["Fetch API is a modern JavaScript interface used to make HTTP requests to servers and retrieve resources such as JSON data images or files fetch is built into modern browsers and returns a promise that resolves to a response object developers often call response.json() to convert the response body into a usable JavaScript object fetch supports multiple HTTP methods including GET POST PUT PATCH and DELETE allowing applications to communicate with backend services fetch requests may include headers request bodies and configuration options fetch is widely used in web applications to load dynamic data submit forms or interact with REST APIs understanding fetch is essential for building modern web applications that rely on server communication"],

18: ["JSON stands for JavaScript Object Notation and is a lightweight text based format used for exchanging data between systems particularly between web browsers and servers JSON is structured similarly to JavaScript objects but follows stricter rules keys must always be enclosed in double quotes and values must be valid JSON data types such as string number boolean null array or object JSON cannot contain functions undefined values or comments making it safe and consistent for data exchange JSON.stringify converts a JavaScript object into a JSON string which can be sent over a network while JSON.parse converts a JSON string back into a usable JavaScript object JSON is one of the most widely used data formats in modern web development and is the standard format for most APIs"],

19: ["Async and await provide modern syntax for working with asynchronous operations in JavaScript making code easier to read and maintain the async keyword is placed before a function declaration which automatically makes the function return a promise inside an async function the await keyword can be used before a promise causing the function to pause execution until the promise resolves this makes asynchronous code appear similar to synchronous code even though it is still non blocking async await greatly simplifies working with promises especially when handling multiple sequential asynchronous operations developers commonly use async await with the fetch API database requests or file operations error handling in async functions is usually implemented using try catch blocks which makes asynchronous code easier to debug and manage"],

20: ["Error handling is an important practice in programming that allows applications to gracefully manage unexpected problems without crashing JavaScript provides the try catch finally structure to handle runtime errors the try block contains code that might produce an error if an error occurs execution immediately jumps to the catch block where the error object can be inspected logged or handled appropriately the finally block runs regardless of whether an error occurred and is often used for cleanup tasks developers can also manually generate errors using the throw keyword which allows creating custom error messages proper error handling improves application stability helps developers identify issues faster and ensures that users receive meaningful feedback instead of broken functionality"]
...
// "HOF: function that returns another function",
...
// "Enables abstraction and reusable logic",
// "Common in functional programming",
// "Helps separate logic from execution",
// "Reduces code duplication",
// "Improves modular and maintainable code",
// "Used in async programming patterns",
// "Examples include setTimeout and event handlers",
// "Forms the basis for patterns like currying and composition"
// ],
...
function getTopicNotes(id) {
  return QUICK_NOTES[id] || ["Notes coming soon."];
}
